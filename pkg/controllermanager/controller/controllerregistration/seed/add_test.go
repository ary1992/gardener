...
var _ = Describe("Add", func() {
	...
	Describe("BackupBucketPredicate", func() {
		...
		Describe("#Generic", func() {
			It("should return true", func() {
				Expect(p.Generic(event.TypedGenericEvent[*gardencorev1beta1.BackupBucket]{})).To(BeTrue())
			})
		})
	})
	...
	Describe("BackupEntryPredicate", func() {
		var (
			p           predicate.TypedPredicate[*gardencorev1beta1.BackupEntry]
		...
		Describe("#Create", func() {
			It("should return false when seed name is not set", func() {
				Expect(p.Create(event.TypedCreateEvent[*gardencorev1beta1.BackupEntry]{Object: backupEntry})).To(BeFalse())
			})

			It("should return true when seed name is set", func() {
				backupEntry.Spec.SeedName = ptr.To("some-seed")
				Expect(p.Create(event.TypedCreateEvent[*gardencorev1beta1.BackupEntry]{Object: backupEntry})).To(BeTrue())
			})
		})

		Describe("#Update", func() {
			It("should return false because new object is no BackupEntry", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.BackupEntry]{})).To(BeFalse())
			})

			It("should return false because old object is no BackupEntry", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.BackupEntry]{ObjectNew: backupEntry})).To(BeFalse())
			})

			It("should return false because neither seed name nor bucket name changed", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.BackupEntry]{ObjectNew: backupEntry, ObjectOld: backupEntry})).To(BeFalse())
			})

			It("should return true because seed name changed", func() {
				oldBackupEntry := backupEntry.DeepCopy()
				backupEntry.Spec.SeedName = ptr.To("new-seed")
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.BackupEntry]{ObjectNew: backupEntry, ObjectOld: oldBackupEntry})).To(BeTrue())
			})
		...
		Describe("#Delete", func() {
			It("should return false when seed name is not set", func() {
				Expect(p.Delete(event.TypedDeleteEvent[*gardencorev1beta1.BackupEntry]{Object: backupEntry})).To(BeFalse())
			})

			It("should return true when seed name is set", func() {
				backupEntry.Spec.SeedName = ptr.To("some-seed")
				Expect(p.Delete(event.TypedDeleteEvent[*gardencorev1beta1.BackupEntry]{Object: backupEntry})).To(BeTrue())
			})
		})

		Describe("#Generic", func() {
			It("should return true", func() {
				Expect(p.Generic(event.TypedGenericEvent[*gardencorev1beta1.BackupEntry]{})).To(BeTrue())
			})
		})
	})
	...
	Describe("ShootPredicate", func() {
		...
		Describe("#Create", func() {
			It("should return false when seed name is not set", func() {
				Expect(p.Create(event.TypedCreateEvent[*gardencorev1beta1.Shoot]{Object: shoot})).To(BeFalse())
			})

			It("should return true when seed name is set", func() {
				shoot.Spec.SeedName = ptr.To("some-seed")
				Expect(p.Create(event.TypedCreateEvent[*gardencorev1beta1.Shoot]{Object: shoot})).To(BeTrue())
			})
		})
		...
		Describe("#Update", func() {
			...
			It("should return false because both old and new networking fields are nil", func() {
				...
				oldShoot := shoot.DeepCopy()
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.Shoot]{ObjectNew: shoot, ObjectOld: oldShoot})).To(BeFalse())
			})
		...
		Describe("#Delete", func() {
			It("should return false when seed name is not set", func() {
				Expect(p.Delete(event.TypedDeleteEvent[*gardencorev1beta1.Shoot]{Object: shoot})).To(BeFalse())
			})

			It("should return true when seed name is set", func() {
				shoot.Spec.SeedName = ptr.To("some-seed")
				Expect(p.Delete(event.TypedDeleteEvent[*gardencorev1beta1.Shoot]{Object: shoot})).To(BeTrue())
			})
		})
	...
	Describe("ControllerInstallationPredicate", func() {
		...
		Describe("#Update", func() {
			It("should return false because new object is no ControllerInstallation", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.ControllerInstallation]{})).To(BeFalse())
			})

			It("should return false because old object is no ControllerInstallation", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.ControllerInstallation]{ObjectNew: controllerInstallation})).To(BeFalse())
			})

			It("should return false because there is no relevant change", func() {
				Expect(p.Update(event.TypedUpdateEvent[*gardencorev1beta1.ControllerInstallation]{ObjectNew: controllerInstallation, ObjectOld: controllerInstallation})).To(BeFalse())
			})
		...
		Describe("#Delete", func() {
			It("should return false", func() {
				Expect(p.Delete(event.TypedDeleteEvent[*gardencorev1beta1.ControllerInstallation]{})).To(BeFalse())
			})
		})

		Describe("#Generic", func() {
			It("should return false", func() {
				Expect(p.Generic(event.TypedGenericEvent[*gardencorev1beta1.ControllerInstallation]{})).To(BeFalse())
			})
		})
	})
