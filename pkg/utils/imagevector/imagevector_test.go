  ...
  - name: "%s"
    ...
    runtimeVersion: "%s"`, image1Src1.Name, *image1Src1.Repository, *image1Src1.Tag, *image1Src1.Tag, *image1Src1.RuntimeVersion)
		...
		Describe("#WithEnvOverride", func() {
			...
			It("should override the ImageVector and return imagePullSecretName from override file", func() {
				var (
					vector       = ImageVector{image1Src3, image2Src1}
					overrideJSON = `
{
	...
	"images": [
		{
			...
			"runtimeVersion": ">= 1.6, < 1.8"
				...
				Expect(err).NotTo(HaveOccurred())
				Expect(mergedVector).To(Equal(ImageVector{image1Src1, image2Src1}))
				Expect(imagePullSecretName).To(PointTo(Equal("my-image-pull-secret")))
			})
