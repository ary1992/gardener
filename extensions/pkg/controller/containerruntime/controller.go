	...
	if args.IgnoreOperationAnnotation {
		if err := ctrl.Watch(
			source.Kind(mgr.GetCache(),
				...
				mapper.TypedEnqueueRequestsFrom[*extensionsv1alpha1.Cluster](ctx, mgr.GetCache(), ClusterToContainerResourceMapper(mgr, predicates...), mapper.UpdateWithNew, ctrl.GetLogger())),
		); err != nil {
			return err
		}
	}

	return ctrl.Watch(source.Kind(mgr.GetCache(), &extensionsv1alpha1.ContainerRuntime{}, &handler.TypedEnqueueRequestForObject[*extensionsv1alpha1.ContainerRuntime]{}, predicates...))
}
